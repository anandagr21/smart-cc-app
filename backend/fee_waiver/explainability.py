...
class WaiverExplainability:
    ...
    def generate_state_explanation(
        ...
        days_remaining: int,
        has_spending_history: bool = True,
    ) -> str:
        if is_achieved:
            return "Annual fee waiver successfully achieved for this cycle."

        if not has_spending_history:
            return (
                f"Not enough spending data to project yet. "
                f"Keep using this card — every purchase counts toward the waiver."
            )
